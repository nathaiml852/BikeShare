...
# extract year and month from the date column and create two another columns 'yr' and 'mnth'
...
def pre_pipeline_preparation(*, data_frame: pd.DataFrame) -> pd.DataFrame:
    """Prepares data for the pipeline."""
    data_frame = get_year_and_month(data_frame)                     # Fetching yr and mnth from 'dteday' column
    
    print("Columns after preprocessing:", data_frame.columns.tolist())  
...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    dataframe = pd.read_csv(Path(f"{DATASET_DIR}/{file_name}"))
    print("Raw dataset columns:", dataframe.columns.tolist())  # ✅ Debug
    transformed = pre_pipeline_preparation(data_frame=dataframe)
    print("Columns after preprocessing:", transformed.columns.tolist())  # ✅ Debug
    return transformed
